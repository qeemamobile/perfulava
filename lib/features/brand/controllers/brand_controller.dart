...
class BrandController extends ChangeNotifier {
  ...
  Future<void> getBrandList(bool reload, {int offset = 1}) async {
    ...
    if (localData != null && offset == 1) {
      log('brand response ${localData.response}');
      var brandList = jsonDecode(localData.response);
      for (var item in brandList) {
        _brandModel?.brands?.add(BrandModel.fromJson(item));
      }
    ...
    if (apiResponse.response?.statusCode == 200) {
      log('brand response ${apiResponse.response!.data}');
      if (offset == 1 && apiResponse.response!.data != null) {
        _brandList.clear();
        _originalBrandList.clear();

        _brandModel?.brands = (apiResponse.response?.data['brands'] as List)
      ...
      } else if (Brand.fromJson(apiResponse.response?.data).brands != null &&
          Brand.fromJson(apiResponse.response?.data).brands!.isNotEmpty) {
        _brandModel = Brand.fromJson(apiResponse.response!.data);
        apiResponse.response?.data['brands'].forEach(
            (brand) => _originalBrandList.add(BrandModel.fromJson(brand)));
        apiResponse.response!.data['brands']
            .forEach((brand) => _brandList.add(BrandModel.fromJson(brand)));
      }
