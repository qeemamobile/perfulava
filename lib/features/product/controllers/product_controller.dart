...
class ProductController extends ChangeNotifier {
  ...
  Future<void> getLatestProductList(int offset, {bool reload = false}) async {
    ...
    if (localData != null &&
        offset == 1 &&
        ProductModel.fromJson(jsonDecode(localData.response)).products !=
            null) {
      _latestProductList = [];
      _latestProductList!.addAll(
          ProductModel.fromJson(jsonDecode(localData.response)).products!);
      _latestPageSize =
          ProductModel.fromJson(jsonDecode(localData.response)).totalSize;
            ...
            if (localData != null && offset == 1) {
              await database.updateCacheResponse(
                  ...
                  CacheResponseCompanion(
                    ...
                    header:
                        Value(jsonEncode(apiResponse.response!.headers.map)),
  ...
  Future<void> getLProductList(String offset, {bool reload = false}) async {
    var localData =
        await database.getCacheResponseById(AppConstants.latestProductUri);
  ...
  Future<void> initBrandOrCategoryProductList(
    ...
    if (offset == 1) {
      _brandOrCategoryProductList?.products?.clear();
    }
    if (isUpdate) {
      notifyListeners();
    }
    ...
    log('you product ${apiResponse.response?.data}');
    if (apiResponse.response?.statusCode == 200) {
      if (offset == 1) {
        print('Product Response: ${apiResponse.response?.data}');
        print('isTrue? before:${_brandOrCategoryProductList == null}');
        // var products  = (apiResponse.response?.data as List).map((e)=> Product.fromJson(e)).toList();
        // _brandOrCategoryProductList?.products?.addAll(products);
        _brandOrCategoryProductList = ProductModel(products: (apiResponse.response?.data as List).map((e)=> Product.fromJson(e)).toList());

        // _brandOrCategoryProductList?.products?.addAll(
        //     ProductModel.fromJson(apiResponse.response?.data).products ?? []);
        print('isTrue? after:${_brandOrCategoryProductList == null}');
        //= ProductModel.fromJson(apiResponse.response?.data);
        print('Product Response: after maping ${apiResponse.response?.data}');
      } else {
        print('Product Response: else  ${apiResponse.response?.data}');
        _brandOrCategoryProductList?.products?.addAll(
            ProductModel.fromJson(apiResponse.response?.data).products ?? []);
        ...
        _brandOrCategoryProductList?.totalSize =
            ProductModel.fromJson(apiResponse.response?.data).totalSize;
        print(
            'Product Response:after maping else  ${apiResponse.response?.data}');
      }
    } else {
      ApiChecker.checkApi(apiResponse);
    }
  ...
  Future<void> getFeaturedProductList(String offset,
    ...
    var localData =
        await database.getCacheResponseById(AppConstants.featuredProductUri);
  ...
  Future<void> getRecommendedProduct() async {
    var localData =
        await database.getCacheResponseById(AppConstants.dealOfTheDay);
  ...
  Future<void> getHomeCategoryProductList(bool reload) async {
    if (_homeCategoryProductList.isEmpty || reload) {
      var localData = await database
          .getCacheResponseById(AppConstants.homeCategoryProductUri);

      if (localData != null) {
        var homeCategoryProductList = jsonDecode(localData.response);
        homeCategoryProductList.forEach((homeCategory) =>
            _homeCategoryProductList
  ...
  Future<void> getMostDemandedProduct() async {
    var localData =
        await database.getCacheResponseById(AppConstants.mostDemandedProduct);

    if (localData != null) {
      mostDemandedProductModel =
          MostDemandedProductModel.fromJson(jsonDecode(localData.response));
      notifyListeners();
    }
  ...
  Future<void> findWhatYouNeed() async {
    var localData =
        await database.getCacheResponseById(AppConstants.findWhatYouNeed);

    if (localData != null) {
      findWhatYouNeedModel =
          FindWhatYouNeedModel.fromJson(jsonDecode(localData.response));
      notifyListeners();
    }
  ...
  Future<void> getJustForYouProduct() async {
    var localData =
        await database.getCacheResponseById(AppConstants.justForYou);
  ...
  Future<void> getMostSearchingProduct(int offset,
      {bool reload = false}) async {
    var localData =
        await database.getCacheResponseById(AppConstants.mostSearching);

    if (localData != null) {
      mostSearchingProduct =
          ProductModel.fromJson(jsonDecode(localData.response));
      notifyListeners();
    }
  ...
  Future<void> getClearanceAllProductList(String offset,
      {bool reload = false}) async {
    var localData = await database
        .getCacheResponseById(AppConstants.clearanceAllProductUri);

    if (localData != null) {
      clearanceProductModel =
          ProductModel.fromJson(jsonDecode(localData.response));
      notifyListeners();
    }

    ApiResponse apiResponse =
        await productServiceInterface!.getClearanceAllProductList(offset);
    ...
    if (apiResponse.response != null &&
      ...
      if (offset == '1') {
        ...
        clearanceProductModel?.products?.addAll(
            ProductModel.fromJson(apiResponse.response!.data).products ?? []);
        clearanceProductModel?.offset =
            (ProductModel.fromJson(apiResponse.response!.data).offset!);
        clearanceProductModel?.totalSize =
            (ProductModel.fromJson(apiResponse.response!.data).totalSize!);
      }
